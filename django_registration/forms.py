...
class TokenFormUniqueEmail(forms.Form):
    ...
    def clean_email(self):
        ...
        """
        try:
            user = OurUser.objects.get(email__exact=self.cleaned_data['email'])
        except OurUser.DoesNotExist:
            return self.cleaned_data['email']
        except OurUser.MultipleObjectsReturned:
            pass
        raise forms.ValidationError(_(u'This email address is already in use. Please supply a different email address.'))

    def save(self, profile_callback=None):
        token = Token.objects.create_token(email=self.cleaned_data['email'])
        return token
...
class RegisterForm(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        try:
            user = OurUser.objects.get(username__exact=self.cleaned_data['username'])
        except OurUser.DoesNotExist:
    ...
    def save(self, token, profile_callback=None):
        """
        ...
        new_user = token.create_user(profile_callback=profile_callback, **self.cleaned_data)
        return new_user
